...
//글의 조회수 추가하기
			...
			closeDBResources(pstmt);  // ejkim.a for warning
			//글번호 검색
		...
		try {
          ...
          //  StringBuffer sql = new StringBuffer();
            ...
            else if(opt.equals("0")) // 제목으로 검색
            {
            	String sql = "SELECT * FROM "
    					+ " (SELECT ROWNUM rnum, B.* FROM "
    									+ " ( SELECT * FROM BOARDE where SUBJECT like ? ORDER BY ref desc, re_step asc ) B ) ";
                sql += " WHERE rnum >= ? and rnum <= ?";
            ...
            else if(opt.equals("1")) // 내용으로 검색
            {
            	String sql = "SELECT * FROM "
    					+ " (SELECT ROWNUM rnum, B.* FROM "
    									+ " ( SELECT * FROM BOARDE where CONTENT  like ? ORDER BY ref desc, re_step asc ) B ) ";
                sql += " WHERE rnum >= ? and rnum <= ?";
            ...
            else if(opt.equals("3")) // 글쓴이로 검색
            {
            	String sql = "SELECT * FROM "
    					+ " (SELECT ROWNUM rnum, B.* FROM "
    									+ " ( SELECT * FROM BOARDE where writer like ? ORDER BY ref desc, re_step asc ) B ) ";
                sql += " WHERE rnum >= ? and rnum <= ?";
